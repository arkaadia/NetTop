...
const MODULES_WORKFLOW: Record<ActiveTab, ModuleWorkflowData> = {
  ...
  'ssh-test': {
    tabId: 'ssh-test',
    nameEn: 'SSH Test & Paramiko 2 Engine',
    nameFa: 'تست SSH و موتور ارتباط واقعی Paramiko 2',
    subtitleEn: 'Live SSH socket tunnels, 8-layer link audits, telemetry extraction, and interactive PTY',
    subtitleFa: 'اتصال واقعی با وب‌سوکت و پارامیکو ۲، ممیزی ۸ لایه فرانت تا بک‌اند، فتچ مشخصات سخت‌افزاری و ترمینال تعاملی',
    icon: Terminal,
    primaryComponent: 'SshTestView.tsx',
    primaryFilePath: 'src/components/sshTest/SshTestView.tsx',
    colorScheme: {
      accent: 'text-emerald-400',
      border: 'border-emerald-500/40',
      bg: 'bg-emerald-500/10',
    ...
    steps: [
      {
        ...
        stepNumber: 1,
        titleEn: 'Device Configuration & Credential Store',
        titleFa: 'پیکربندی تجهیز و ذخیره امن مشخصات',
        descEn: 'Configure hostname, port, username, password/private-key, and verify target credentials.',
        descFa: 'تعریف آدرس IP یا نام سرور، پورت، نام کاربری و رمز یا کلید خصوصی SSH با رمزنگاری معتبر.',
        component: 'SshDeviceModal.tsx',
        componentPath: 'src/components/sshTest/SshDeviceModal.tsx',
        apiEndpoint: '/api/ssh-test/devices',
        apiMethod: 'POST',
        hardwareLayer: 'Remote Network Node (TCP 22 / SSH)',
        targetToken: '[بخش: ssh تست | کامپوننت: src/components/sshTest/SshDeviceModal.tsx | مرحله: ثبت تجهیز]',
      },
      {
        id: 'diagnostic_audit',
        stepNumber: 2,
        titleEn: '8-Layer End-to-End Diagnostic Audit',
        titleFa: 'ممیزی لایه‌به‌لایه ارتباط فرانت تا بک‌اند (۸ لایه)',
        descEn: 'Inspect each layer from browser REST gateway, Python runtime, WS tunnel, DNS, TCP handshake, SSH banner, Paramiko auth, to PTY allocation.',
        descFa: 'بررسی تک‌تک لایه‌های ارتباطی و اعلام دقیق محل و علت بروز اختلال به همراه راهکار پیشنهادی رفع مشکل.',
        component: 'DiagnosticModal.tsx',
        componentPath: 'src/components/sshTest/DiagnosticModal.tsx',
        apiEndpoint: '/api/ssh-test/devices/:id/diagnose',
        apiMethod: 'POST',
        hardwareLayer: 'End-to-End Audit (Node.js ↔ Python ↔ TCP Socket ↔ SSH Layer)',
        targetToken: '[بخش: ssh تست | کامپوننت: src/components/sshTest/DiagnosticModal.tsx | مرحله: ممیزی ۸ لایه]',
      },
      {
        id: 'data_fetching',
        stepNumber: 3,
        titleEn: 'Paramiko 2 Live Telemetry Discovery',
        titleFa: 'فتچ و استخراج اطلاعات زنده دیوایس',
        descEn: 'Probes real OS kernel, uptime, network interfaces, memory, and disk usage via authentic Paramiko 2 commands with zero mock data.',
        descFa: 'اجرای دستورات واقعی بر روی دیوایس جهت خواندن کرنل، زمان روشن بودن، اینترفیس‌ها و سخت‌افزار بدون داده‌های ساختگی.',
        component: 'FetchedDataModal.tsx',
        componentPath: 'src/components/sshTest/FetchedDataModal.tsx',
        apiEndpoint: '/api/ssh-test/devices/:id/fetch',
        apiMethod: 'POST',
        hardwareLayer: 'Remote Operating System Shell (Linux/Cisco/MikroTik)',
        targetToken: '[بخش: ssh تست | کامپوننت: src/components/sshTest/FetchedDataModal.tsx | مرحله: فتچ دیتا]',
      },
      {
        id: 'interactive_pty_terminal',
        stepNumber: 4,
        titleEn: 'Interactive WebSocket Terminal Session',
        titleFa: 'شل تعاملی اینتراکتیو با وب‌سوکت و Paramiko',
        descEn: 'Real-time bidirectional PTY shell supporting full VT100/ANSI keys, shortcuts, and interactive commands.',
        descFa: 'ترمینال زنده و تعاملی دوطرفه با پشتیبانی از کلیدهای میانبر، تاریخچه دستورات و خروجی رنگی ANSI.',
        component: 'InteractiveTerminalModal.tsx',
        componentPath: 'src/components/sshTest/InteractiveTerminalModal.tsx',
        apiEndpoint: '/ws/ssh-test',
        apiMethod: 'WS',
        hardwareLayer: 'Paramiko invoke_shell() PTY Worker Process',
        targetToken: '[بخش: ssh تست | کامپوننت: src/components/sshTest/InteractiveTerminalModal.tsx | مرحله: ترمینال تعاملی]',
      },
    ],
  },
};
