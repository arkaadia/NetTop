...
export async function testRealSsh(params: import('../types').RealSshTestRequest): Promise<import('../types').RealSshTestResult> {
  ...
  if (!res.ok) {
    const errData = await res.json().catch(() => ({ message: 'SSH testing service unreachable' }));
    throw new Error(errData.message || 'SSH connection failed');
  }
  return res.json();
}
